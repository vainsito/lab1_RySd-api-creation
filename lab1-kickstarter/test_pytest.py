...
def mock_response():
    with requests_mock.Mocker() as m:
        ...
        m.get('http://localhost:5000/peliculas/70', status_code=404, json={'mensaje': 'Id no encontrado'})
        ...
        m.get('http://localhost:5000/genero/Drama', status_code=201)
        m.get('http://localhost:5000/peliculas/genero/No_existe', status_code=404, json={'mensaje': 'Género no encontrado'})

        # Simulamos la respuesta para obtener peliculas por una palabra clave
        m.get('http://localhost:5000/peliculas/titulo/Indi', status_code=201)
        m.get('http://localhost:5000/peliculas/titulo/No_existe', status_code=404, json={'mensaje': 'Pelicula no encontrado'})

        # Simulamos la respuesta para obtener una pelicula aleatoria
        m.get('http://localhost:5000/peliculas/random', status_code=201)
        m.get('http://localhost:5000/peliculas/random/No_existe', status_code=404, json={'mensaje': 'No hay peliculas disponibles'})

        # Simulamos la respuesta para obtener una pelicula aleatoria de un genero particular
        m.get('http://localhost:5000/peliculas/random/Drama', status_code=201)
        m.get('http://localhost:5000/peliculas/random/No_existe', status_code=404, json={'mensaje': 'Género no encontrado'})

        # Simulamos la respuesta para obtener una pelicula aleatoria de un genero particular en un dia feriado
        m.get('http://localhost:5000/peliculas/feriado/Drama', status_code=201)
        m.get('http://localhost:5000/peliculas/feriado/No_existe', status_code=404, json={'mensaje': 'Género no encontrado'})
...
def test_actualizar_detalle_pelicula_error(mock_response):
    datos_actualizados = {'titulo': 'Nuevo título', 'genero': 'Comedia'}
    response = requests.put('http://localhost:5000/peliculas/60', json=datos_actualizados)
    assert response.status_code == 404
...
def test_eliminar_pelicula_error(mock_response):
    response = requests.delete('http://localhost:5000/peliculas/60')
    assert response.status_code == 404

def test_peliculas_genero(mock_response):
    response = request.get('http://localhost:5000/peliculas/genero/Drama')
    assert response.status_code == 201

def test_peliculas_genero_error(mock_response):
    response = request.get('http://localhost:5000/peliculas/genero/No_existe')
    assert response.status_code == 404

def test_filtro_por_titulo(mock_response):
    response = request.get('http://localhost:5000/peliculas/titulo/Indi')
    assert response.status_code == 201
    assert "Indi".lower() in response.json()['titulo'].lower()

def test_filtro_por_titulo(mock_response):
    response = request.get('http://localhost:5000/peliculas/titulo/No_existe')
    assert response.status_code == 404

def test_pelicula_random(mock_response):
    response = request.get('http://localhost:5000/peliculas/random')
    assert response.status_code == 201

def test_pelicula_random_error(mock_response):
    response = request.get('http://localhost:5000/peliculas/No_existe')
    assert response.status_code == 404

def test_pelicula_random_genero(mock_response):
    response = request.get('http://localhost:5000/peliculas/random/Drama')
    assert response.status_code == 201
    assert response.json()['Genero'] == Drama

def test_pelicula_random_genero_error(mock_response):
    response = request.get('http://localhost:5000/peliculas/random/No_existe')
    assert response.status_code == 404

def test_peli_random_feriado(mock_response):
    response = request.get('http://localhost:5000/peliculas/feriado/Drama')
    assert response.status_code == 201
    assert response.json()['Genero'] == Drama

def test_peli_random_feriado_error(mock_response):
    response = request.get('http://localhost:5000/peliculas/feriado/No_existe')
    assert response.status_code == 404